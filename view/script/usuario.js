$(document).ready(function(){
	...
	$("#usuarioregistrar").click(function(){
		var nombres=$("#nombres").val(),
			...
			if (nombres.length==0 || apellidop.length==0 || apellidom.length==0 ||
				correo.length==0 || dni.length==0 || direccion.length==0 || contrasena.length==0 ||
				repitecontrasena.length==0) {
				toastr.info("Ingresar los datos respectivos","Usuario");
			}else if (dni.length!=8) {
				toastr.warning("El dni tiene que tener 8 digitos","Usuario");
			}else if (contrasena!=repitecontrasena) {
				toastr.warning("la contraseña no coincide","Usuario");
			}else{
				$.post("../controller/usuario.php?boton=insertar",{nombres:nombres, apellidop:apellidop,
					apellidom:apellidom, correo:correo, dni:dni, direccion:direccion,
					contrasena:contrasena, repitecontrasena:repitecontrasena, tipo_usuario:tipo_usuario}, function(rsp){
					if (rsp=="1") {
						toastr.success("Se Registro exitosamente","Usuario");
	...
	$(document).on("click", ".actua", function(){
		...
		$.ajax({
			...
			$("#tipo_usuarioedit").val(data.tipo_usuario);
		});
	});
	...
	$("#btnedita").click(function(){
		var idusuario=$("#idusuario").val(),
			...
			tipo_usuario=$("#tipo_usuarioedit").val();
			if (dni.length!=8) {
				toastr.warning("El dni tiene que tener 8 digitos","Usuario");
			}else if (contrasena!=repitecontrasena) {
				toastr.warning("Las contraseña no coinciden","Usuario");
			}else{
				$.post("../controller/usuario.php?boton=editar",{idusuario:idusuario, nombres:nombres, apellidop:apellidop,
						apellidom:apellidom, correo:correo, dni:dni, direccion:direccion,
						contrasena:contrasena, repitecontrasena:repitecontrasena, tipo_usuario:tipo_usuario}, function(rsp){
				if (rsp=="1") {
					toastr.success("Se Actualizo exitosamente","Usuario");
