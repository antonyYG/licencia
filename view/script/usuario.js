$(document).ready(function () {
    ...
    $("#btnedita").click(function () {
        ...
        if (dni.length != 8) {
            ...
            $.post(
                "../controller/usuario.php?boton=editar",
                {
                    idusuario: idusuario,
                    nombres: nombres,
                    apellidop: apellidop,
    ...
    $(document).on("click", "#deshabilitarespe", function () {
        ...
        $.post(
            "../controller/usuario.php?boton=inactivo",
            { idusuario: idusuario },
            function (rsp) {
                if (rsp == "1") {
                    toastr.success("El usuario está deshabilitado", "Usuario");
                    $("#modalinactivoEspe").modal("hide");
                    table.ajax.reload();
                } else {
                    toastr.error("No se pudo deshabilitar", "Usuario");
                }
            }
        );
    });
    ...
    $(document).on("click", "#habilitarespe", function () {
        ...
        $.post(
            "../controller/usuario.php?boton=activo",
            { idusuario: idusuario },
            function (rsp) {
                if (rsp == "1") {
                    toastr.success("El usuario está habilitado", "Usuario");
    ...
    $("#modalinsertarUser").on("hidden.bs.modal", function () {
        limpiar();
    });

    $("#modaleditarUser").on("hidden.bs.modal", function () {
        limpiarEdit();
    });
});
